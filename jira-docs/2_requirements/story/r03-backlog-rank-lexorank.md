...
> **나중에 드러난 결함(LJ-48, 2026-08-03).** 이 스토리가 닫힌 뒤, 자기 관리 보드에 이슈 47건을
> 적재하다 **끝에 덧붙일 때마다 랭크가 정확히 2자씩 자란다**는 것이 드러났다. `between(before, null)`이
> 상한 없음을 all-`z`로 읽고 진짜 중점을 잡은 탓이다: `hzzzzz`(6) → `qzzzzzhi`(8) → … → 14번째에
> 32자 한도. 그 뒤로는 `nextBacklogRank`가 직전 랭크를 재사용해 **15번째부터 전부 같은 랭크**를
> 공유했고, 이 보드도 41건이 한 랭크에 몰려 `duplicateRankRegions=1`을 상시 보고하고 있었다.
>
> 생성이 실패하지도, 정합성이 깨지지도 않았다 — `(rank, uid)` 타이브레이크가 순서를 지켰다.
> **그래서 아무 테스트도 잡지 못했다.** 이 스토리의 인수조건은 "사이에 끼워 넣기"와 "리밸런스"를
> 확인했지 **가장 흔한 연산인 '맨 뒤에 추가'를 반복했을 때 무슨 일이 나는지**는 묻지 않았다.
>
> 끝으로의 추가를 이등분이 아니라 **계수**로 바꿨다(`append`). 고정 폭에서 `BASE`만큼 더하므로
> 길이가 6자에 머물고, 폭 하나로 20억 건을 감당한다. 삽입 사이 간격은 `STEP=BASE`가 남긴다.
> 형식 변경이 아니라 값 선택의 변경이라 마이그레이션이 필요 없다.
