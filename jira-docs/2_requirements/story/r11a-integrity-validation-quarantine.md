...
- [x] Given 머지 결과 ACTIVE 스프린트가 2건 이상, When 감지하면, Then **프로젝트 전체를 "스프린트 충돌" 상태**로 표시하고 스프린트 시작·종료 명령을 **409**로 차단한다. 이슈 조회·편집·상태 전이는 계속 동작한다.  
      **✅ 해소: r05b(Sprint 02 Wave 2)에서 시작·종료 명령이 생기며 차단까지 구현됐다**
