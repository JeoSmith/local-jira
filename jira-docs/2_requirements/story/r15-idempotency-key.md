...
>
> ⚠ **분해 문제 — AC1·AC5가 이번 스프린트에서 닫히지 않는다.** 두 인수조건이
> `POST .../comments`와 `POST /runs`를 지목하는데, 전자는 **M4(r19)** 이고 후자는 이 스프린트
> **Wave 3(r17a)** 이다. 멱등성 기계 자체는 어느 생성 라우트에나 붙는 래퍼로 구현했으므로
> `POST /issues`는 지금 닫혔고, `/runs`는 r17a에서 한 줄로 연결된다. **코멘트 부분은 M4까지
> 닫을 수 없다** — 구현 실패가 아니라 스토리 분해가 아직 없는 API를 인수조건에 넣은 것이다.
