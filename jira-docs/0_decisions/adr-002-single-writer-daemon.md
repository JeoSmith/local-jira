...
**해제 시점은 플랫폼마다 다르다.** macOS/BSD는 잠금이 죽은 프로세스 자신의 FD에 있으므로
커널이 즉시 떨군다. Linux는 `flock(1)` 헬퍼 **별도 프로세스**가 들고 있어서, 부모가 죽은 뒤
헬퍼가 파이프 EOF를 관측하고 종료할 때까지 짧은 창이 남는다. 부하가 걸리면 관측될 만큼 길다 —
크래시 직후 재기동한 서버가 "다른 서버가 이미 실행 중"이라는 **거짓 안내를 받는다**.
그래서 Linux 경로는 경합을 3초까지 재시도한 뒤에야 `E_BOOTSTRAP_BUSY`로 단정한다.
살아 있는 홀더는 그 창을 넘겨 살아 있으므로 여전히 busy로 보고되고, 정리 중이던 홀더는
창이 닫히기 전에 사라진다.
