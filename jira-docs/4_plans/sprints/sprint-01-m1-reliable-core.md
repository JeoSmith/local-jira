...
| **3** | r08c · r02a · r02b · r03 · r14b | 20 | ✅ **완료** |
| **4** | r11a · r04a | 11 | ✅ **완료** |

**76 / 76점 (100%)** · 테스트 316건 통과 · CI는 ubuntu·macOS 양쪽 초록.
**Sprint 01 커밋 범위 완료.**
...
**r14b 완료 — Wave 3 종료.** 이벤트 인덱스에 `before`/`after`/`initiated_by`가 빠져 있어
`schema_version`을 2로 올렸다(인덱스는 파생 캐시라 재빌드로 끝난다).

타임라인은 이슈 본문이 아니라 `GET /issues/{key}/activity`다 — children·links와 같은 이유로,
본문 해시가 ETag이므로 매번 자라는 목록을 넣으면 validator가 계속 움직인다. 정렬은
`(at, event_id)`인데, 시각이 초 단위라 같은 초의 두 변경이 흔하고 두 번째 키가 없으면
**클론마다·재빌드마다 순서가 달라진다**.

주체 표기는 색만이 아니라 **글자를 함께** 쓴다(사람/에이전트/외부 편집/시스템). 색만으로는
색각 이상 사용자와 흑백 스크린샷에서 구분이 사라지는데, §8이 요구한 건 정확히
"에이전트 변경이 사람 변경처럼 보이지 않을 것"이다.

카드 배지는 **마지막 변경 주체**이고 `created_by_kind`는 따로 유지한다. 브라우저에서
외부 편집 후 배지가 "외부 편집"으로 바뀌는 것까지 확인했다.

미정 2건 해소: 페이지네이션은 기본 50건·`before` 커서, 종류별 필터는 **제공하지 않는다**
(한 이슈 타임라인은 대개 전부 보이고, 필터는 "안 보이는 항목이 있다"는 오해를 만든다).

### Wave 4 — 무결성과 필터

**r11a 완료.** 격리는 두 단계로 갈린다. Stage A는 **파일 하나**를 못 읽는 경우(파서가 내는
`conflict_marker`·`yaml_error` 등을 그대로 기록), Stage B는 **집합 전체**를 봐야 아는 경우
(`duplicate_uid`·`dangling_ref`·`cycle`)다. 참조가 끊겼는지·uid가 겹치는지는 어느 한 파일의
성질이 아니라 집합의 성질이고, 머지는 정확히 대부분의 파일을 건드리지 않은 채 집합을 바꾼다.

작업 중 드러난 것 셋:

- **파싱 실패가 레코드를 지우고 있었다.** 호출부가 `clearFile` 후 로드해서, 파싱이 실패하면
  기존 행이 사라진 채 남았다. AC는 "격리하되 삭제하지 않는다"이므로 `loadScannedFile`이
  **파싱 성공을 확인한 뒤에** 지우도록 순서를 바꿨다. 이제 실패하면 이전 행이 `INVALID`로
  남고 마지막 정상 hash도 보존된다.
- **격리된 이슈가 404를 내고 있었다.** 조회가 `state='OK'`로 거르니 격리된 이슈는 "없음"으로
  보였다. 없어진 게 아니라 **숨겨진 것**이므로, 삭제를 찾아 헤매게 만들지 않도록 격리 검사를
  not-found 검사보다 **먼저** 하게 했다.
- **conflict marker 검사를 두 번째로 구현할 뻔했다.** 파서에 이미 있었고 줄 번호까지 준다.
  중복 구현은 서로 어긋나기만 하므로 걷어냈다.

미정이던 상태코드는 **409**로 확정했다. 423(Locked)은 다른 주체가 점유했다는 뜻인데 격리는
아무도 잡고 있지 않다 — 기다려도 풀리지 않고 사람이 파일을 고쳐야 풀린다.

**r04a 완료 — Sprint 01 종료.** 필터 규칙은 **한 파라미터 안에서는 OR, 파라미터끼리는 AND**로
통일했다. 필드마다 다르면(라벨은 AND, 상태는 OR) 사용자가 매번 시험해 봐야 믿을 수 있다.

페이지네이션은 **커서**다. 목록은 재정렬되는 것이 목적이므로 offset은 순서가 바뀐 뒤 행을
말없이 건너뛰거나 중복시킨다. 정렬 키 `(backlog_rank, uid)` 자체를 인코딩하고, 손으로 만들어
offset처럼 쓰지 못하도록 불투명하게 뒀다. 재정렬 중 페이징에서 중복이 없는 것을 테스트로 고정했다.

`sprint=active`는 예약어로 인정하되 **400으로 거부**한다. 값 필터로 흘려보내면 빈 목록이
나오고, 에이전트는 그걸 **"할 일이 없다"로 읽는다** — 없는 것과 아직 없는 것은 다르게 답해야 한다.

`claimable`은 **양방향**을 본다. `A blocks B`와 `B blocked_by A`는 같은 관계이므로(S1-D4)
한쪽만 보면 차단자의 절반이 보이지 않는다.

5,000건 복합 필터 **p95 5ms**(AC13 예산 300ms).
